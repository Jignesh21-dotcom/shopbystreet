...
import { getStreetBySlug } from '@/lib/cacheHelpers';
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.localstreetshop.com/cities/${resolvedParams.city}/${resolvedParams.street}/${resolvedParams.shop}/products/${resolvedParams.product}/request`,
    },
  };
}
...
export default async function RequestOrderPage({ params }: PageProps) {
  ...
  const productsHref = `/cities/${citySlug}/${streetSlug}/${shopSlug}/products`;
  const shopHref = `/cities/${citySlug}/${streetSlug}/${shopSlug}`;
  ...
  if (streetError || !streetData) {
    return (
      <UnavailableState
        message="The street for this Order Request could not be found."
  ...
  if (!cityData || normalizeSlug(cityData.slug || '') !== citySlug) {
    return (
      <UnavailableState
        message="The city for this Order Request could not be verified."
        backHref={productsHref}
      />
    );
  }
  ...
  if (shopError) {
    return (
      <UnavailableState
        message="The shop for this Order Request could not be loaded."
        backHref={productsHref}
      />
    );
  }
  ...
  if (!shop) {
    return (
      <UnavailableState
        message="This shop could not be found."
        backHref={productsHref}
      />
    );
  }
