...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!result.data) {
    return { title: 'Shop Not Found | LocalStreetShop India' };
  }
  ...
  const title = `${shop.name} – ${street?.display_name || street?.name}, ${city?.name} | LocalStreetShop India`;
  const description =
    shop.description ||
    `Discover ${shop.name} in ${city?.name}, ${state?.name}, India on LocalStreetShop.`;
  const canonical = `https://www.localstreetshop.com/countries/india/${stateSlug}/${citySlug}/streets/${streetSlug}/${shopSlug}`;
...
export default async function IndiaShopPage({ params }: PageProps) {
  ...
  const streetHref = `/countries/india/${stateSlug}/${citySlug}/streets/${streetSlug}`;
  ...
  const mapQuery = encodeURIComponent(
    `${shop.name}, ${shop.address || street?.name}, ${city?.name}, ${state?.name}, India`,
  );
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        ...
        <section className="mt-6 overflow-hidden rounded-[2rem] border border-orange-100 bg-white shadow-xl">
          ...
          <div className="bg-gradient-to-r from-orange-600 to-green-700 p-8 text-white sm:p-10">
            ...
            {shop.category && (
              <p className="mt-3 text-lg text-orange-50">{shop.category}</p>
            )}

            <div className="mt-7 flex flex-wrap gap-3">
              <Link
                href={productsHref}
                className="rounded-full bg-white px-6 py-3 font-bold text-orange-700 transition hover:bg-orange-50"
              >
                🛍 View Products
              </Link>
              {!isClaimed && (
              ...
              <div className="mt-4 flex flex-wrap gap-3 text-sm font-bold">
                {(shop.phone || shop.contact) && (
                  <a
                    ...
                    className="rounded-full bg-white px-4 py-2 text-orange-700"
                  >
                    Call Shop
                  </a>
                )}
