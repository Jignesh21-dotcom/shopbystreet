...
// India addresses can contain Block No., shop/unit numbers, fractions such as 71/2,
        ...
        <section className="overflow-hidden rounded-[2rem] border border-orange-100 bg-white shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-4 border-t border-orange-100 bg-white p-5 sm:grid-cols-3">
            <div className="rounded-3xl bg-orange-50 p-5">
              <p className="text-3xl font-extrabold text-orange-700">{shops.length}</p>
        ...
        {addressGroups.length > 0 && (
          <section className="mt-8 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-sm font-bold uppercase tracking-[0.2em] text-orange-700">Street Walk Preview</p>
              ...
              <Link href={firstStopHref} className="rounded-full bg-orange-600 px-6 py-3 text-center font-bold text-white transition hover:bg-orange-700">
                🚶 Start Walking →
              </Link>
            </div>
                ...
                >
                  <div className="mb-6 flex flex-col gap-4 border-b border-slate-100 pb-5 md:flex-row md:items-start md:justify-between">
                    <div>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {Object.entries(categoryCounts).map(([category, count]) => (
                          <span key={category} className="rounded-full bg-orange-50 px-3 py-1 text-sm font-semibold text-orange-700">
                            {category} ({count})
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      ...
                      <Link href={nextHref} className="rounded-full bg-orange-600 px-5 py-2 text-center text-sm font-bold text-white transition hover:bg-orange-700">
                  ...
                  <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
                    {group.shops.map((shop) => {
                      ...
                      return (
                        <Link
                          key={shop.id}
                          href={`/countries/india/${state}/${city}/streets/${street}/${shop.slug}`}
                        ...
                        >
                          ...
                          <div className="p-5">
                            <h3 className="text-xl font-extrabold text-slate-950 group-hover:text-orange-700">{shop.name}</h3>
