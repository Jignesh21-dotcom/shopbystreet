...
export default async function USCityPage({ params }: Props) {
  ...
  const { data: country } = await supabase.from('countries').select('id').eq('slug','united-states').maybeSingle();
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-12"><div className="mx-auto max-w-6xl">
    <Link href={`/countries/united-states/${state.slug}`} className="font-bold text-blue-700">← {state.name}</Link>
    <section className="mt-6 rounded-[2rem] bg-white p-8 shadow-sm"><p className="text-sm font-bold uppercase tracking-[0.2em] text-blue-600">{state.name}, United States</p><h1 className="mt-3 text-4xl font-black">Explore {city.name}</h1><p className="mt-3 text-slate-600">Choose a street, market or local shopping area.</p></section>
    <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{(streets||[]).map((street)=><Link key={street.id} href={`/countries/united-states/${state.slug}/${city.slug}/streets/${street.slug}`} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-black">{street.display_name || street.name}</h2><p className="mt-2 text-blue-700 font-bold">Walk this street →</p></Link>)}</section>
  </div></main>;
}
