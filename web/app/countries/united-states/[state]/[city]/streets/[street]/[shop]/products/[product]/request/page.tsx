...
type ShopRow = {
  ...
  street: {
    city: {
      state: { name: string; slug: string } | { name: string; slug: string }[] | null;
    } | { state: { name: string; slug: string } | { name: string; slug: string }[] | null }[] | null;
  } | { city: { state: { name: string; slug: string } | { name: string; slug: string }[] | null } | { state: { name: string; slug: string } | { name: string; slug: string }[] | null }[] | null }[] | null;
};
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.localstreetshop.com/countries/united-states/${resolvedParams.state}/${resolvedParams.city}/streets/${resolvedParams.street}/${resolvedParams.shop}/products/${resolvedParams.product}/request`,
    },
  };
}
...
export default async function RequestOrderPage({ params }: PageProps) {
  ...
  const stateSlug = normalizeSlug(
    decodeURIComponent(resolvedParams.state || ''),
  );
  ...
  const productsHref = `/countries/united-states/${stateSlug}/${citySlug}/streets/${streetSlug}/${shopSlug}/products`;
  const shopHref = `/countries/united-states/${stateSlug}/${citySlug}/streets/${streetSlug}/${shopSlug}`;

  const { data: shopData, error: shopError } = await supabase
    .from('shops')
    .select(`id, name, slug, street:streets!inner(slug, city:cities!inner(slug, state:provinces!inner(name, slug, country:countries!inner(slug))))`)
    .eq('slug', shopSlug)
    .eq('approved', true)
  ...
  if (shopError || !shopData) {
    return <UnavailableState message="This United States shop could not be found." backHref={productsHref} />;
  }

  const shop = shopData as unknown as ShopRow;

  const streetRelation: any = Array.isArray((shop as any).street) ? (shop as any).street[0] : (shop as any).street;
  const cityRelation: any = Array.isArray(streetRelation?.city) ? streetRelation.city[0] : streetRelation?.city;
  const stateRelation: any = Array.isArray(cityRelation?.state) ? cityRelation.state[0] : cityRelation?.state;
  const stateName = stateRelation?.name || stateSlug.replace(/-/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase());
