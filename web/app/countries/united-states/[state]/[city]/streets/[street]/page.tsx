...
import USStreetClient from './USStreetClient';
...
export default async function USStreetPage({ params }: Props) {
    ...
    return (
      <main className="min-h-screen bg-slate-50 px-4 py-16">
        <div className="mx-auto max-w-3xl rounded-3xl bg-white p-8">
          ...
          <Link href={`/countries/united-states/${stateSlug}/${citySlug}`} className="mt-5 inline-block font-bold text-blue-700">
            ← Back to city
          </Link>
        </div>
      </main>
  ...
  if (shopsError) {
    console.error('Unable to load United States street shops:', shopsError);
  }
