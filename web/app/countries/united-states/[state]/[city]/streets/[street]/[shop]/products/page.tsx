...
export async function generateMetadata({
  ...
  return {
    title: `Products from ${shopSlug.replace(/-/g, ' ')} | LocalStreetShop United States`,
    description:
      'Browse products from this local U.S. business on LocalStreetShop.',
    alternates: {
      canonical: `https://www.localstreetshop.com/countries/united-states/${resolvedParams.state}/${resolvedParams.city}/streets/${resolvedParams.street}/${resolvedParams.shop}/products`,
    },
  };
}

export default async function USShopProductsPage({
  params,
}: PageProps) {
  const resolvedParams = await params;
  ...
  const shopHref =
    `/countries/united-states/${stateSlug}/${citySlug}` +
    `/streets/${streetSlug}/${shopSlug}`;

  /*
   * Load the street first and verify that it belongs to:
   *
   * United States → requested state → requested city → requested street
   *
  ...
  const { data: streetData, error: streetError } = await supabase
    ...
    .eq('city.state.country.slug', 'united-states')
    .maybeSingle();

  if (streetError) {
    console.error(
      'U.S. products street lookup failed:',
      streetError,
    );
  }
  ...
  const hasCorrectHierarchy =
    ...
    normalizeSlug(stateRecord?.slug || '') === stateSlug &&
    normalizeSlug(countryRecord?.slug || '') === 'united-states';
   ...
   *
   * No U.S.-specific shop table is required.
   */
  ...
  if (shopError) {
    console.error(
      'U.S. products shop lookup failed:',
      shopError,
    );
  }
  ...
  if (productsResult.error) {
    console.error(
      'Failed to fetch United States shop products:',
  ...
  if (settingsResult.error) {
    console.error(
      'Failed to fetch United States shop order settings:',
      ...
      <section className="mx-auto max-w-6xl">
        ...
        <div className="rounded-3xl border border-blue-100 bg-white p-6 shadow-sm md:p-8">
          ...
          <p className="mt-3 max-w-3xl text-gray-600">
            Browse products from this U.S. business. Prices are
    ...
    <main className="min-h-screen bg-gradient-to-b from-orange-50 via-white to-white px-4 py-12">
      <div className="mx-auto max-w-4xl rounded-3xl border border-red-100 bg-white p-8 text-center shadow-sm">
        ...
        <p className="mt-3 text-gray-600">
          This shop could not be found on the selected United States street.
        </p>

        <Link
          href={`/countries/united-states/${stateSlug}/${citySlug}/streets/${streetSlug}`}
