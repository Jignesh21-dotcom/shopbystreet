...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: result ? `Explore ${result.state.name} | LocalStreetShop United States` : 'State Not Found | LocalStreetShop United States' };
}
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-12">
      <div className="mx-auto max-w-6xl">
        <Link href="/countries/united-states" className="font-bold text-blue-700">← United States</Link>
        <section className="mt-6 rounded-[2rem] bg-gradient-to-br from-blue-800 to-slate-800 p-8 text-white">
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-blue-100">LocalStreetShop United States</p>
        ...
        <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {result.cities.length ? result.cities.map((city) => (
            <Link key={city.id} href={`/countries/united-states/${result.state.slug}/${city.slug}`} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
