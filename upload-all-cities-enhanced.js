...
// 🔁 City ID map for trusted 52 cities (replace below with generated full list)
const CITY_IDS = {
...
async function processCity(filePath, city) {
  ...
  if (ways.length === 0) {
    console.warn(`⚠️ Skipping ${city} — no usable ways found`);
    return;
  }
  ...
  for (const way of ways) {
    try {
      ...
      await withTimeout(uploadStreet({
        name: way.tags.name,
        slug: cleanSlug,
        city,
        city_id,
